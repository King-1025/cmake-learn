	...
	for (unsigned int i = 1; i < elf_hdr->e_shnum; i++) {
		...
		if (section_header_entry->sh_type == SHT_DYNAMIC) {
			...
			for (unsigned int j = 0; j < dynamic_section_entries; j++) {
				...
				case DT_FLAGS_1: removed_name = "DT_FLAGS_1"; break;
					case DT_ENTRY_0: removed_name = "DT_ENTRY_0"; break;
...
int main(int argc, char const** argv)
{
	...
	if (argc < 2 || (argc == 2 && strcmp(argv[1], "-h")==0)) {
		...
		fprintf(stderr, "       Processes ELF files to remove DT_FLAGS_1, DT_ENTRY_0, DT_VERNEEDED, DT_VERNEEDNUM, DT_RPATH\n"
